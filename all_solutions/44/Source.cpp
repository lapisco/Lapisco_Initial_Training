...
int main()
{
  ...
  unsigned int result = cvLabel(thresholded, labelImg, blobs);
	...
	cvFilterByArea(blobs,1,10000); //filtrar a area para pegar os contornos. Ex usado: só pega os contornos de area 1 até 10000
  ...
  for (CvBlobs::const_iterator it=blobs.begin(); it!=blobs.end(); ++it)
  {
   ...
   vContourPolygon *polygon = cvConvertChainCodesToPolygon(&(*it).second->contour);
